...
#      return
    ...
    result =
      phs: []
      pages:  JSON.parse(String(body).slice 14, -1).photos.pages
    ...
    for photo in photos
      ...
      try
        request.get req_uri, (error, rsp, data) ->
          ...
          if result.phs.length == photos.length
            console.log "Last geo info recieved: " + result.phs.length
            res.send result
      catch e
        console.log e
...
#TODO: получать pages от flickr и потом при очередной загрузке выбирать рандомную страницу
#TODO: test, refactor
